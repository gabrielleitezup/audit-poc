...
func (main CircleRepository) SaveWithContext(ctx context.Context, entity models.Circle) error {
	return main.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {

		if err := tx.Model(&models.Circle{}).Create(&entity).Error; err != nil {
			logrus.Errorln("Create circle error:", err)
			return err
		}
		...
		if err := tx.Model(&auditions.Audition{}).Create(&audit).Error; err != nil {
			logrus.Errorln("Create circle audit error:", err)
			return err
		}
